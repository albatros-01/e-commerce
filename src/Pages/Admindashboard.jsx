...
const Admindashboard = () => {
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center bg-gray-100 p-6">
      <div className="w-full max-w-5xl bg-gray-500 rounded-2xl shadow-lg p-6">
        <header className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-2xl ml-100 font-extrabold">Admin Dashboard</h1>
          </div>
        </header>
              ...
              {admincart.length === 0 ? (
                ...
                admincart.map((cur, i) => (
                  <tr key={i} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm text-gray-700">{cur.name}</td>
                    <td className="px-4 py-3">
                      {cur.img ? (
                        <img src={cur.img} alt={cur.name} className="w-12 h-12 object-cover rounded" />
                      ) : (
